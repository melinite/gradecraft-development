...
class AssignmentWeightTest < ActiveSupport::TestCase
  ...
  test "calculates total assignment weight when current weight is persisted" do
    ...
    assert_equal 6, @assignment_weight.total_assignment_weight
  end

  test "calculates total assignment weight when current weight is not persisted" do
    create_assignment_weight :weight => 1, :assignment => create_assignment
    create_assignment_weight :weight => 2, :assignment => create_assignment
  ...
  test "adds an error when total student weight exceeds course total assignment weight" do
