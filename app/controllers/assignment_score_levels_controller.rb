class AssignmentScoreLevelsController < ApplicationController
  ...
  def index
    @assignment_score_levels = AssignmentScoreLevel.all
  end

  def show
    @assignment_score_level = AssignmentScoreLevel.find(params[:id])
  end
  ...
  def create
    @assignment_score_level = AssignmentScoreLevel.new(params[:assignment_score_level])
